...
$(document).ready(function() {
	$("#banner-nav li.manage_teachers").addClass('active');
...
function tryloadData(){	
	$.ajax({
 		type:'POST',
 		url:'${pageContext.request.contextPath}/sys/users/allTeachers.json',
 		data:{},
...
function renderTeachersTable(data){	
	$('#teachers_table>tbody>tr').remove();
	rows_data=data.data;		 
	var opts= 
	...
	$.each(rows_data,function(i,row){
		var arr=[row.id,row.code,row.username,row.department,opts]
		$('#teachers_table').append(generate_html_tr(arr));
		$('#teachers_table button.edit:last').click(function(){
			showEditModal(row.id);
		});
		$('#teachers_table button.dele:last').click(function(){
...
function showEditModal(row_id){	 
	 if (row_id){
		 ...
		 $('#user_edit_modal form input[name="main_role_name"]').val('teacher');
	 }	
			...
			<div class="panel-body">
				<div class="bottom-right">
					<button type="button" class="btn btn-default bottom-left-btn" id="btn_user_add">
						<i class="glyphicon glyphicon-plus"></i>&nbsp;新增教师账户
					</button>				
				</div>
				<table class="table table-condensed" id="teachers_table">
					<thead>
						<tr>
							<th>#</th>
							<th>工号</th>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      	...
      	<div class="form-group">
				...
				<p class="col-sm-10"><label class="control-label">教师</label></p>
			</div>
			<div class="form-group">
			...
			<input name="main_role_name" type="hidden" value="teacher">
		</form>     
      </div><!-- /.modal-body -->
